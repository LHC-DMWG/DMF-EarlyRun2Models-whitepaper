...
To study the dependence of kinematic distributions on the coupling strength, samples were generated where a pair of $\mDM=10$~\gev DM particles is produced on-shell from the mediator of $\mMed=1$~\tev. 
...
\centering
\subfloat[]{
\includegraphics[width=0.65\textwidth]{figures/monojet/mxx}
\label{fig:monojet_mchichi_a}}
\hfill
\subfloat[]{
\includegraphics[width=0.65\textwidth]{figures/EFT/TruthShat3_compEFTvsZprime_DM150_narrow_new}
...
DM masses in Fig.\,\ref{fig:monojet_VAmodels}. %The minimal width between the samples with the same DM and mediator masses differ only at the percent level.
...
%For scalar and pseudoscalar mediators, the coupling to DM determines the form of the matrix element which explains the similarity of the samples with the same type DM couplings. 
...
\subsection{Scalar and pseudoscalar mediator, \schannel exchange}
...
%% The following has been removed during review
...
%The model assumes Dirac DM particles and is based on the minimal flavor violation (MFV), which motivates Higgs-like Yukawa couplings of the mediator to the SM quarks. No other couplings, such as to leptons, are allowed in this model.
...
\begin{figure}
	...
	\caption{Minimal width as a function of mediator mass for scalar and pseudo-scalar mediator assuming couplings of 1. The total width is shown as solid lines for DM masses of \mDM=10~\gev, 30~\gev, 100~\gev and 300~\gev in black, red, brown and green, respectively. The individual contributions from DM are indicated by dotted lines with the same colors. The contribution from all quarks but top is shown as magenta dotted line and the contribution from top quarks only is illustrated by the dotted blue line. The dotted beige line shows the contribution from the coupling to gluons. The dotted black line shows the extreme case $\Gamma_{\rm{min}}=\mMed$.}
...
\subsubsection{\texorpdfstring{Additional considerations for $t \bar{t}$ and $b \bar{b}$+\MET signatures}{Additional considerations for ttbar/bbbar+\MET signatures}}
...
\end{eqnarray}
%However, it is important to keep in mind that there is no simple scaling rule for how the cross section changes with the DM mass, mediator mass and the mediator width because PDFs matter in such cases as well.
...
\begin{figure}
	...
	%Vector (scalar) mediator is shown at the top (bottom), the left (right) column corresponds to $\mMed=100$~\gev ($\mMed=1$~\tev). DM mass of 10~\gev is considered.}
