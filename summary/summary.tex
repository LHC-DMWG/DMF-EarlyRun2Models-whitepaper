...
%\documentclass[a4,debug,notitlepage]{dmfm}
%%\documentclass[11pt,twoside,a4paper,debug]{dmfm}
\documentclass[notitlepage,12pt,twoside,a4paper,debug]{dmfm}
...
mediators, included as a separate factor with respect to \gq:
...
models (\mdm, $m_{med}$, \gq, and \gdm) and three parameters
...
width up to $\Gamma \approx m_{med}$.  (The exceptional case of a narrow,
off-shell mediator is addressed later.)
...
\hline
 ...
 10   & 10 & 20$^{*}$ & 50 & \ditto    & \ditto    & \ditto    & \ditto
 & \ditto    &  \ditto    &  \ditto             & \textbf{3000} & 5000  \\
 50   & 10 &   & 50 & 100$^{*}$ & 150 &  \ditto   &  \ditto   &  \ditto   &  \ditto    &  \ditto             & \textbf{3000} & 5000  \\
 150  & 10 &  &  &  &  & 200 & 300$^{*}$ & 500 &  \ditto    &  \ditto             & \textbf{3000} & 5000  \\
 200  & 10 &    &    &     &     & 200 & 300 & 500  &  \ditto   &  \ditto             & \textbf{3000} & 5000  \\
...
but not for the search including heavy quarks. The points
marked by an asterisk ($^*$) will be shifted above/below threshold for production.}
...
samples per model, covering \mmed from 10 GeV to 3 TeV (10 GeV to 1.5 TeV) for the vector and axial vector (scalar and pseudoscalar) models,
and \mdm from 5 GeV to 1.5 TeV.
...
signals should be generated with couplings in a grid of $\gdm = \gq = $ 1.0, 0.3, 0.5, 1.45,
while for the scalar and pseudoscalar signals the recommended grid is
$\gdm = \gq = $ 1.0, 0.1, 0.5, 1.0, 2.0, 3.0.
...
these through simple rescaling of the total rate, using cross-section
...
There are matrix element implementations of the $s$-channel
vector and scalar mediated DM production
available in POWHEG, MADGRAPH and MCFM.
We recommend the POWHEG implementation of DM pair production +1 parton at NLO
(the DMV model), 
...
both collaborations. Once ready, ATLAS and CMS should discuss whether this can be harmonized.]}
...
The $t$-channel model is implemented in MadGraph.  It is recommended
...
with a colored scalar resonance coupling to Dark Matter and a $b-$quark 
as described in Ref.~\cite{Agrawal:2014una}.
...
Preliminary studies show that generation of a grid of roughly 10 points
in DM mass, 15 points in mediator
...
$b\bar b$+MET
signatures are possible.  The details for the implementation of these
...
is above or below the $t\bar t$ threshold. 
...
\hline
\end{tabular}
\caption{Simplified model benchmarks for $t\bar{t}$+DM production via spin-0 mediators decaying to Dirac DM fermions taking the minimum width prescription for $g_v = g_{DM} = 1$.}
\label{tab:ttdm_benchmarks}
\end{table}
...
Models of single top+MET production are different from the other models under consideration, since they truly involve a single top quark in the final state.
...
\begin{itemize}
 ...
 \item mediator-quark \gq = $\frac{1}{3}$.
...
$t\bar t$, and dilepton final states, are thus a crucial part of the LHC
