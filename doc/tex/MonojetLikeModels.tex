...
While the recommendations are general,
the discussions in this Chapter are focused on the monojet final state. 
...
These studies and those in subsequent sections are based on simulation of the models using the implementations recommended in Appendix~\ref{app:MonojetLikeModels_Appendix}. 
...
between the spin-1 mediator $\Zprime$ and SM and DM fields, with
...
For simplicity, we will sometimes refer to these as
the models \modelDMV and \modelDMA, respectively.
...
\Todo{change reference here from page to subsection?}
...
In this section, the differences between the models \modelDMV and \modelDMA
are described.
...
\Todo{This can be understood in the eikonal approximation since the A terms have an additional $v^2$ in front.}
...
No significant differences between the samples with same type Dark Matter coupling are seen, given the statistical precision of the generated samples.
This is expected since the mediator is produced on-shell, and the details of the invisible decay are unimportant.
...
From Figure~\ref{fig:monojet_scan_V_g}, we
observe no difference in the shapes of the \MET distribution for the different choices of the coupling strength.
This is a generator-level prediction with no kinematic selections or detector simulation. Coupling values in the scan range 0.1--1.45, fixing $\gq=\gDM$, correspond to a rough estimate of the lower sensitivity of mono-jet analyses and a maximum coupling value such that $\Gamma_{\rm{min}} < \mMed$.
...
not peculiar.   See the Appendix for details. \Todo{reference to place in appendix}

%\Todo{move this paragraph to appendix (and leave the previous one here as it talks about 5 TeV samples that we recommend and compares the shape to EFT)}
...
\end{figure}
\Todo{Now these figures seem out of place.   The point of the first plot seems to be that the EFT limit is only recovered for g=1.}
...
simplified model is suppressed and the $\MET$ spectrum hardens, as demonstrated with the choice of \mDM=1~\tev in the same plot. Figure\,\ref{fig:monojet_scan_V_mDM100} demonstrates that the $\MET$ spectrum hardens further with increasing \mDM, accompanied by the gradual decrease of the cross section. From these observations one can conclude:
\begin{itemize}
...
is illustrated in Fig.\,\ref{fig:monojet_scan_V_mMed1000}. Therefore, a coarse binning in $\mMed$ is sufficient at $\mMed \ll 2\mDM$.
...
\Todo{Clarify shape of model repository}
%for the other one.
...
The discussion of parameters for the model with a color-singlet, spin-0 mediator
