...
The three main benchmark simplified models suggested in this Chapter for 
early Run-2 searches   

The primary simplified models suggested in this Chapter for Dirac fermion DM 
comprise \spinzero and \spinone mediators. Section~\ref{sec:monojet_V} covers the
...
is provided in Table~\ref{tab:models}.
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{figures/monojet/mxx.eps}
\includegraphics[width=0.45\textwidth]{figures/monojet/TruthShat_compEFTvsZprime_DM150_narrow.eps}
\caption{Invariant mass of the Dark Matter pair in the vector mediator samples with $\mDM=10$~\gev, $\mMed=5$~\tev and different coupling strengths (left).
Similar comparison is shown for the samples with different mediator masses considering $\Gamma_{\rm{min}}/\mMed=0.05$. An EFT sample is also displayed in the latter case. The distributions are normalised to unit area.}
\label{fig:monojet_mchichi}
\end{figure}
\Todo{describe the plots (update the right one) and add back the paragraph relevant for the choice of the sample resembling EFT!!!}
...
%29 
mass points each. One point at very high mediator mass (10~\tev) is added for each of the DM masses scanned, to aid the reinterpretation of results in terms of contact interaction operators (EFTs) as discussed in Section~\ref{sec:RecommendationEFTResults}. 
