...
and the second where we consider only a pseudoscalar for which the associated scalar is assumed to be decoupled at higher energies. Note that this assumption does not hold in a UV complete model where the two components of the complex scalar mediator would be approximately degenerate.
...
\todo{Clarify shape of model repository} for the other one. No preference is given between the two models as they have the
