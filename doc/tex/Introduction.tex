%\svnidlong
%{$HeadURL: svn+ssh://doglioni@svn.cern.ch/reps/LHCDMF/trunk/doc/tex/Introduction.tex $}
%{$LastChangedDate: 2015-06-30 22:05:35 +0200 (Tue, 30 Jun 2015) $}
%{$LastChangedRevision: 434 $}
%{$LastChangedBy: doglioni $}
%\svnid{$Id: Introduction.tex 434 2015-06-30 20:05:35Z doglioni $}

Dark matter (DM) \sidenote{Many theories of physics beyond the Standard Model predict the existence
...
%CD: discussions, discussion, discussions. Needs rewording
...
DM-electroweak boson couplings \sidenote{An example of a dimension-5 operator for scalar
...
%For example, consider the DM coupling to a SM Higgs singlet mediator, $S$, whose vacuum expectation value gives mass to a heavy vector-like fermion, 
%$F$, which also carries SM quantum numbers. The $F$ triangle graph then couples $S$ to the SM gauge boson pair. 
%Thus $\chi \bar{\chi} \rightarrow S \rightarrow F\bar{F} \text{(loop)} \rightarrow G G$ as required.
...
%CD: this sentence does not fit here anymore
%It should also be possible to map results from simplified models to contact operators,
%in the high mediator mass limit. 
...
%CD: no literature here, or we have to cite >100 papers. 
