\section{Implementation of \schannel and \tchannel models for \MET+X analyses}
...
in Ref.\,\cite{Alwall:0706.2569}, we recommend using the CKKW-L matching scheme.
...
either flavour scheme used for DM+$b\bar{b}$ production, we recommend the 4-flavor scheme to follow
...
\end{minipage}
\caption{Comparison of the jet multiplicity (left) and angular correction $\Delta R(j_1, j_2)$ (right) for the DM+$b\bar{b}$ scalar model generated in the 4-flavour and 5-scheme.
	The samples are generated for $m_\chi=1$~GeV and $m_\phi=10$~GeV.
	\label{fig:4Fvs5F}}
\end{figure}

\section{Implementation of specific models for $V+\MET$ analyses}
