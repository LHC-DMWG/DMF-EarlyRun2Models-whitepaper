...
Since \madgraph provides a flexible and easy to use framework for implementing new models via the {\sc FeynRules} package, it is highly relevant to comment on parton matching which is important to consider whenever events with jets in the final state are produced. The parton matching is performed in order to avoid double counting of the partons from matrix elements and parton showering in \pythiaEight.
Several parton matching techniques are available.
Based on the comparative study in Ref.\,\cite{Alwall:0706.2569}, there is some advantage to using the CKKW-L matching scheme.
The text below describes in detail the implementation of the CKKW-L matching in \madgraph and \pythiaEight. On an example of mono-jet EFT model, it also provides a template study of finding the optimal matching scale for the event generation and discusses the impact of the parton emission multiplicity chosen at the matrix element level. Similar studies may be performed for the MLM matching.
