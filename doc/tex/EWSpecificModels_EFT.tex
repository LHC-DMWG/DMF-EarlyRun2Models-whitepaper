...
the case, we recommend only generation of simplified model result as that can be reinterpreted.}. 
...
%CD: here I'm being too vague?
In the spirit of the Forum consensus to privilege simplified models,
model-independent results as in Appendix~\ref{app:Presentation_Of_Experimental_Results} should be privileged
over considering these operators as realistic benchmarks.
However, another consideration that emerged from the Forum discussion was that the EFT approach allowed
more model-independence for reinterpretations, and it is still worth having
results available in terms of these operators. We devote the end of this Section to 
provide a recommendation on how to present the results from this model, including an assessment of their  
validity using a conservative procedure that is only dependent on EFT parameters.
...
are the fields of the gauge bosons, $\chiDM$ denote the Dark Matter fields
and $\Lambda_5$ is the effective field theory scale~\Todo{The nomenclature in different models differs from literature: 
	$\Lambda_5$ vs $\lambda$}. Note that these operators are of true dimension 7, 
...
In such models the full theory may be expected to contain additional operators with Higgs-Dark Matter couplings~\ref{Djouadi:2012zc}.
...
\Todo{Waiting for plot by Bhawna Gomber showing this}
...
\Todo{Include unpublished study of rescaling for dimension-8 operators by N. Whallon in dimension-8 section.}
%From Andy
...
\todo{The axes of these plots will be rescaled in terms of c1/c2.}
...
are negligible when changing the coefficients $c_1$ and $c_2$, as shown
in Figure~\ref{fig:EFTD7_EW_kinematics}. Only the case $c_1=c_2=1$ is generated as benchmark;
...
%    \hfill
  \subfloat[Missing transverse momentum distribution.\label{fig:EFTD7_EW_Z_MET}]{%
  	\includegraphics[width=0.7\textwidth]{figures/EW/monoZhad_SP/metPt}
  }
  \hfill
  \subfloat[Acceptance.\label{fig:EFTD7_EW_Z_acc}]{%
  	\includegraphics[width=0.7\textwidth]{figures/EW/monoZhad_SP/accS}
  }
...
of the DM as $y_{\chiDM} = \frac{1}{\Lambda^{4}}$.Todo{Is more detail needed here?}
...
\todo{Include comment about UV completion of EFT Dim 7 with loops. Uli is working on this.}
...
dimension-7 operators are particularly challenging since their completion involves loops~\footnote{
...
A model with a spin-1 mediator cannot be considered as an candidate for completion either, since dimension-7 operators
