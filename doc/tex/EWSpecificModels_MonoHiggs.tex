...
\Todo{AB: remove the extra Z in the diagram for the MonoHZprimeqq}
...
\begin{figure}[!h!tpd]
	...
	\caption
	{
		Feynman diagrams of leading order processes leading to Higgs+MET events: 
                ...
                a model with a vector mediator ($Z^\prime$) 
...
and
		...
		from the decay of an on-shell resonance $Z^\prime$ giving rise to a Higgs+MET signature
...
for models with low pseudoscalar mediator masses ($m_{Z_B'} = 100$ GeV, $m_{Z^\prime} = 1$ TeV, $m_{A^0} = 100$ GeV)
...
We recommend to produce signal events for a fixed $g_z=0.8$, $\tan{\beta}=1$ and $M_{DM}=100$~GeV. For these values, we scan the 2-D parameter space of ${M_{Z^\prime}, M_{A^0}}$ with $M_{Z^\prime}=600, 800, 1000, 1200, 1400$~GeV, and $M_{A^0}=300, 400, 500, 600, 700, 800$~GeV with $M_{A^0} < M_{Z^\prime}-m_h$, for a total of 24 points. The choice of scan is justified by the sensitivity study in~\cite{Berlin:2014cfa}: the expected LHC sensitivity for Run-2 is up to $M_{Z^\prime} \sim 1.5$~TeV.
