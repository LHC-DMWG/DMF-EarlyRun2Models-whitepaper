...
A dark matter candidate \DMParticle{} and a new particle (vector or scalar) 
...
\begin{itemize}
	\item resonant production, as shown in the diagram of Fig.~\ref{fig:feyn_prod} (a), where a color triplet scalar $\varphi$ or vector field are exchanged in the s-channel, and decay into a spin 1/2 invisible new fermion $\chi$ and a top quark;
...
a coloured scalar resonance $\varphi$ decaying into a top quark and a spin-$1/2$ fermion $\chi$ (a),
...
where $u$ ($d$) stands for any $up$ ($down$)-quark, 
the index $S$ stands for scalar field, and the index $q$ 
...
$a^q_{\{S\}R}$ (scalar couplings) and $b^q_{\{S\}R}$ (pseudoscalar couplings)
...
either a new scalar or a new vector ($V$). For simplicity, we only consider the case of a vector new particle, 
...
symmetric matrices in flavor space $a^{\{1\}}_{FC}$ for the vector couplings
...
\item particle name: the dark matter candidate $\chi$ is not implemented %(as this model assumes $\BR{V}{\chi\chi}=100\%$)
\end{itemize}
...
Since they only show kinematic quantities related to the mediator, these figures are relevant independently of the $V$ decay mode (be it visible or invisible). \Todo{Pictures will be improved in the next drafts}.
\begin{figure}[!h!tpd]
	\centering
