...
\Todo{Insert example of figure using ATLAS limits from PUB note, applying the two truncation methods.}
...
\begin{figure}[hbpt!]
	...
	\subfloat[\MET{}]{
		...
		\hfill
	\subfloat[Center of mass energy $E_cm$]{
	...
	\caption{Comparison of the kinematic distributions for a narrow \schannel mediator (equivalent to \gq = 0.25 and \gDM = 1), 
		at generator level for a jet+\MET{} signature. 
		\label{fig:EFT_kinematics}}
\end{figure}

\Todo{Insert figure with varying \mDM, showing we need to simulate the various DM masses.}
