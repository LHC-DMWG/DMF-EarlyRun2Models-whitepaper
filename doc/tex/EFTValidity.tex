...
“a regime where careful treatment is necessary, 
since the EFT may not capture features of models with finite mediator masses.2  In particular, it is the events corresponding to the highest momentum
transfer which are the most likely to differ between the EFT treatment and a simplified model
containing a light mediator.  Therefore, whenever EFT models are used and especially in comparison to constraints from other experiments~\footnote{
...
, collider experiments should apply a truncation procedure which removes the highest momentum transfer events from the analysis (according to one of the several prescriptions in the literature~\cite{Busoni:2013lha,Busoni:2014sya,Busoni:2014haa,Aad:2015zva,Racco:2015dxa})
and should present their results as a function of the truncation parameter.”
