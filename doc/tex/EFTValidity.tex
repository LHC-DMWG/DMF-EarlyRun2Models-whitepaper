...
Most of this report has focused on simplified models as
an alternative to EFTs.  Here, we wish to emphasize that
Effective Field Theories (EFTs) can remain a useful
tool in the interpretation of DM searches at the LHC.
...
As we have learned, EFTs do not truly represent a model-independent representation of the data for the current collider sensitivity.   Ideally, experimental results would be cast into bounds of allowed signal events in different kinematic regions.   Until this is done systematically by the analyses, we present
several prescriptions in the literature
that can be used to quote bounds on EFT operators.

For the uninitiated, we illustrate the problem with EFTs by considering an effective interaction
