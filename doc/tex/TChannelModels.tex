...
The preceding sections address models containg a Dirac WIMP coupled to the SM through exchange of a neutral vector or scalar mediator in an $s$-channel process. A $t$-channel process may couple the SM and DM directly, leading to a different phenomenology. Under the assumption that $\chi$ is a Standard Model (SM) singlet, the mediating particle, labeled $\phi$, is necessarily charged and coloured. 
...
in Fig. \ref{fig:tchannelMonojet}. An interesting point of difference
with the $s$-channel simplified models is that the mediator can
radiate a SM object, such as a jet or gauge boson. The forum has also
studied MET+$Z$ production where, in diagrams of the first type, one
of the initial quarks radiates $Z$ boson instead of a gluon, or the
boson is radiated off the internal scalar line \todo{cite published
  ATLAS analysis}. This model can also give a signal in the di-jet +
MET channel when, for example, the $\chi$ is exchanged in the
$t$-channel and the resulting $\phi$ pair each decay to a jet +
$\chi$. Fig.~\ref{tchannelDijet} shows the leading order diagrams.

The model is parallel to, and partially motivated by, the squark of
the MSSM, but in this case the $\chi$ is chosen to be Dirac as
above. A second, crucial different with respect to the MSSM is the
...
to be very small within the MSSM. In the more general theories
...
should also study analyses directly optimized for the diagrams which
...
\begin{figure}
  ...
  \begin{feynmandiagram}[modelTmonojetE]
    ...
    \fmfdot{v1,v2,v3}
  \end{feynmandiagram}
\caption{Leading order mono-jet $t$-channel processes, adapted from {\cite:Papucci:2014}.}\label{fig:tchannelMonojet}
\end{figure}
...
\begin{figure}
  ...
  \begin{feynmandiagram}[modelTDijetE]
    ...
    \fmfdot{v1,v2,v3,v4}
  \end{feynmandiagram}
\caption{Leading order two-jet $t$-channel processes, adapted from {\cite:Papucci:2014}.}\label{fig:tchannelDijet}
\end{figure}

\newthought{Parameter scans}

Ref.~\cite{Pappuci:2014} studies the parameter space and obtains
...
Ref.~\cite{Pappuci:2014}, except in the heavy mediator limit ($M_{\phi} \approx 2$\,TeV). A scan over the width was not available for this report; thus we recommend scanning a range of possible widths as discussed in a more-limited way for the $s$-channel mono-jet, spanning from the minimal width to a value approaching the particle limit (for example, $\Gamma \approx M_{\phi}/3$ in Ref.~\cite{Pappuci:2014}).

\newhthought{Implementation}

The MadGraph implementation of the Ref.~\cite{Pappuci:2014} is
available from the Forum repository \todo{cite}, following the
