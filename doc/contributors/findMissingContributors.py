...
def getLastFirstName (fullName) :
	...
	if len( splitName ) == 3 :
		if (splitName[1] == "de" or splitName[1] == "De" or splitName[1] == "du" or
                    ...
                    splitName[1] == "Alcaraz" or splitName[1] == "Rojas" or
...
#open the file
dmwg_contributors_list = []
dmwg_contributors_list_file = open('members-lhc-dmwg-contributors.csv','r')
for line in dmwg_contributors_list_file:
    firstPartOfEmail = line.split("@")[0].split(",")[len(line.split("@")[0].split(","))-1]
    lastPartOfEmail = line.split("@")[1].split(",")[0].strip()
    dmwg_contributors_list.append(firstPartOfEmail+"@"+lastPartOfEmail)

#print out the emails and check them against the list
listOfRecipients = ""

for key, value in SortedNameEmailInstitutionDic.iteritems() :

    if value[0] not in dmwg_contributors_list :
        listOfRecipients = listOfRecipients + value[0] + ","

print listOfRecipients
